	    ...
	    struct thread **ret)
{
	...
	/* Since only one thread per process this will get the pid of parent.
	*  If there were more it would think its parent was a thread of same process.
	*/
...
void
thread_wakeup_head(const void *addr, struct queue *cv_queue)
{

	int i, result;
	struct thread *head = q_remhead(cv_queue);
	...
	for (i=0; i<array_getnum(sleepers); i++) {
		...
		if (t->t_sleepaddr == addr && head == t) {
			...
			/*
			*  Makes sure we do not run this loop again.
			*/
...
/*
 * Will return 0 when the newly created thread exits.
 */
int
thread_join(struct thread *child_thread, int *status)
{
	int has_exited = 0;
	...
	if (has_exited == 0){
		...
		has_exited = pidlist_search(child_thread->pid, status);
		...
		//Case 2b pid exists and wait for it to exit
			...
			thread_sleep(child_thread->pid); // <---- Going to sleep on pid
			splx(spl);
		}
	}	
	int spl = splhigh();

...
/*
	for (i=0; i<eh.e_phnum; i++) {
		...
		topten = ph.p_vaddr>>22;
		btmten = ph.p_vaddr & 0x3FFFFF;
		btmten = btmten >> 12;
		int index = curthread->t_vmspace->firstpt[topten][btmten];
		//get the right physical address that represents the mapping of the virtual address.
		paddr_t coremap_pa = freeaddr+(PAGE_SIZE*index);
	
		result = load_segment (v, ph.p_offset, PADDR_TO_KVADDR(coremap_pa), 
				      ph.p_memsz, ph.p_filesz,
				      ph.p_flags & PF_X);

		//result = page_read(v, ph.p_offset, ph.p_vaddr, 
				//      ph.p_memsz, ph.p_filesz,
				   //   ph.p_flags & PF_X);
		if (result) {
			return result;

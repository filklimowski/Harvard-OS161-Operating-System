...
static int ref;
static int read=0;
struct lock *paginglock;


void
vm_bootstrap(void)
{
	paginglock = lock_create("paginglock");
	swap_init();
	coremap_init();
...
int find_fifo (void){
	...
	u_int32_t secs = 0xffffffff, toptenold, btmtenold;
	
	for (i = 3; i < max_page_num; i ++){
		toptenold = 0;
		btmtenold = 0;
		if (coremap[i].time <= secs && coremap[i].addr != NULL && coremap[i].locked != LOCK_BIT){
			vaddr_t new_va = coremap[i].va;
			toptenold = new_va >>22;
			btmtenold = new_va & 0x3FFFFF;
			btmtenold = btmtenold >> 12;
			if (coremap[i].addr->firstpt[toptenold] != NULL && coremap[i].addr->firstpt[toptenold][btmtenold]!= 0){
				secs = coremap[i].time;
				index_lowest = i;
			}
		}
...
int evict (vaddr_t vaddr) { //vaddr is the virtual address we want to put into the coremap and give a frame
	...
	coremap[to_swap].addr->firstpt[toptenold][btmtenold] = swap_index*2 | SWAP_PT; //set as->firstpt[t][b] to show it is swapped out
	...
	for (i=0; i<NUM_TLB; i++) {
		TLB_Read(&ehi, &elo, i);
		if ((ehi & TLBHI_VPAGE) == swap_holder[swap_index].va) {
...
int swap_in (vaddr_t vaddr, int swap_index) {
	...
	vaddr_t va_toswap = swap_holder[swap_index].va; //the va to swap out
	u_int32_t toptenold, btmtenold;
	toptenold = va_toswap >>22;
	btmtenold = va_toswap & 0x3FFFFF;
	btmtenold = btmtenold >> 12;
	swap_holder[swap_index].addr->firstpt[toptenold][btmtenold] = index*2 & CORE_PT;
...
vm_fault(int faulttype, vaddr_t faultaddress) //TLB holds a limited number of virtual to physical translations. If it cant be translated by the TLB. results in a fault. Also write to read only causes a fault. These faults are caught here
{
	...
	if (faultaddress >= vbase1 && faultaddress < vtop1) { // Text
		...
		PAGE_SIZE, tmp_flsz);
		coremap[as->firstpt[toptenbits][btmtenbits]/2].locked = LOCK_BIT;
		if (result) {
			return result;
		}
...
u_int32_t toptenbits1, btmtenbits1, toptenbits2, btmtenbits2;
	...
	int *secondpt = kmalloc(1024*sizeof(int));
		as->firstpt[toptenbits1] = secondpt;
	...
	{		
		...
		u_int32_t counter, tmp_flsz, tmp_off;
		counter=0;
		while (as->as_vbase1+i*PAGE_SIZE - counter*PAGE_SIZE > as->as_vbase1)
			counter ++;
		tmp_flsz = as->filesize_text - counter * PAGE_SIZE;
		tmp_off = as->off_text + counter * PAGE_SIZE;
		int result = page_read(as->as_vnode, tmp_off, PADDR_TO_KVADDR(temp),
						PAGE_SIZE, tmp_flsz);
			if(result) {
				return result;
			}
	...
	int *secondpt2 = kmalloc(1024*sizeof(int));
		as->firstpt[toptenbits2] = secondpt2;
	...
	{	
		...
		u_int32_t counter, tmp_flsz, tmp_off;
		counter=0;
		while (as->as_vbase2+i*PAGE_SIZE - counter*PAGE_SIZE > as->as_vbase2)
			counter ++;
		if (as->filesize_data!= 0) {
			tmp_flsz = as->filesize_data - counter * PAGE_SIZE;
			tmp_off = as->off_data + counter * PAGE_SIZE;
...
as_copy(struct addrspace *old, struct addrspace **ret)
{
	...
	int i, index;
	paddr_t paddr;
	u_int32_t toptenbits, btmtenbits, toptenbits1, btmtenbits1, toptenbits2, btmtenbits2;
	new = as_create();
	if (new==NULL) {
		return ENOMEM;
	}
	...
	for(i=0;i<old->as_npages1;i++)	{	
		memmove((void *) (PADDR_TO_KVADDR(coremap[new->firstpt[1][i]/2].pa) ) , (const void *)old->as_vbase1+(i*PAGE_SIZE),PAGE_SIZE);
	}  	
	for(i=0;i<old->as_npages2;i++) {		
		memmove((void *) (PADDR_TO_KVADDR(coremap[new->firstpt[64][i]/2].pa) ), (const void *)old->as_vbase2+(i*PAGE_SIZE),PAGE_SIZE);
	}
	...
	toptenbits = old->as_stackvbase>>22;

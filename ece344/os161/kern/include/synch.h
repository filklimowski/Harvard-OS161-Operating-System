...
struct lock {
	char *name;
	volatile struct thread *thread_addr;		// Thread address
...
struct cv {
	char *name;
	struct queue *cv_queue;  // Link list for whats waiting on the Condition Variable
	// add what you need here
	// (don't forget to mark things volatile as needed)
};
...
void	   cv_lockrelease(struct cv *cv, struct lock *lock);

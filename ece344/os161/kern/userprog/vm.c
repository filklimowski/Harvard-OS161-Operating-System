...
/*
u_int32_t coreswap_insert (u_int32_t vaddr, u_int32_t offset, unsigned long npages) {
...
vm_fault(int faulttype, vaddr_t faultaddress) //TLB holds a limited number of virtual to physical translations. If it cant be translated by the TLB. results in a fault. Also write to read only causes a fault. These faults are caught here
{
	...
	int probe = TLB_Probe(faultaddress, 0);
		


	if(faultaddress==1073741824) kprintf("invalid address");
	//TLB_Probe
	...
	if (faultaddress >= vbase1 && faultaddress < vtop1) {
		...
		if(faulttype==0) {
			...
			int result = page_read(as->as_vnode, tmp_flsz, PADDR_TO_KVADDR(coremap[as->firstpt[toptenbits][btmtenbits]].pa),
							PAGE_SIZE, as->filesize_data);
			if(result) {
					return result;
			}
		}/*
		else if(faulttype==1) {
			int result = page_write(as->as_vnode, tmp_flsz, PADDR_TO_KVADDR(coremap[as->firstpt[toptenbits][btmtenbits]].pa),
							PAGE_SIZE, tmp_off);
			if(result) {
					return result;
			}
...
as_copy(struct addrspace *old, struct addrspace **ret)
{
	...
	//fill new's coremap with old coremap's pages

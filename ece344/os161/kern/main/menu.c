...
static
void
cmd_progthread(char **ptr, int nargs)
{
	char **args = ptr;
	char progname[128];
	int result;
...
common_prog(int nargs, char **args)
{
	...
	child_ptr->pid = get_newpid();

	struct pid_list *new_pid;
	new_pid = pidlist_create(child_ptr->pid);
	if (new_pid == NULL){
		return -1;
	}
	...
	thread_join(child_ptr, &exitcode);
...
/**
 * The Debugging command line. Checks the 2nd argument to ensure either 
 * on or off has been entered or else sends an error message. If it is 
 * a correct inpur then the code checks to see if the correct bit is 
 * either high or low then adds or subtracts accordingly.
 */	
static
int
cmd_df(int nargs, char **args)
{
	if (  (nargs != 3) || (atoi(args[1]) < 1) || (atoi(args[1]) > 12)  )
...
static const char *opsmenu[] = {
	"[s]       Shell                     ",
	"[p]       Other program             ",
	"[dbflags] Debug flags                ", //added to be shown in menu
...
//added this to be the menu that shows up when user types dbflags at menu prompt
...
//this prints the dbflags menu by calling showmenu
static
int
...
static struct {
	...
	{ "s",		cmd_shell },
	{ "p",		cmd_prog },
	{ "df",		cmd_df }, //added to recognize df operation
	{ "mount",	cmd_mount },
	{ "unmount",	cmd_unmount },
	{ "bootfs",	cmd_bootfs },
	{ "pf",		printfile },

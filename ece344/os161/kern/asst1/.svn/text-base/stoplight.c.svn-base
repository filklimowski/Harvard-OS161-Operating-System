...
struct directionsquare *printout;
...
{
	while (printout == SQUARELOCKED)
		cv_wait(cv_print, print_lock);
	lock_acquire (print_lock);
	printout = SQUARELOCKED;

        kprintf("%s car = %2d, direction = %s, destination = %s\n",
                msgs[msg_nr], carnumber,
                directions[cardirection], directions[destdirection]);

	printout= SQUAREUNLOCKED;
	cv_signal(cv_print, print_lock);
           ...
           char ** args)
{
	...
	printout = directionsquare_create(0);
	cv_print = cv_create("print");
	print_lock = lock_create("printout");
